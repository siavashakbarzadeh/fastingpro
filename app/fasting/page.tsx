...
const MOCK_SESSIONS: FastSession[] = [
    { id: '1', date: '2025-12-20', plan: '16:8', start: '20:00', end: '12:00', durationHours: 16.2, goalHours: 16, isGoalMet: true },
    { id: '2', date: '2025-12-21', plan: '16:8', start: '21:00', end: '13:00', durationHours: 15.5, goalHours: 16, isGoalMet: false },
    { id: '3', date: '2025-12-22', plan: '18:6', start: '19:00', end: '13:00', durationHours: 18.0, goalHours: 18, isGoalMet: true },
    { id: '4', date: '2025-12-23', plan: '18:6', start: '20:00', end: '14:00', durationHours: 18.2, goalHours: 18, isGoalMet: true },
    { id: '5', date: '2025-12-24', plan: '16:8', start: '20:30', end: '12:30', durationHours: 16.5, goalHours: 16, isGoalMet: true },
    { id: '6', date: '2025-12-25', plan: '16:8', start: '21:00', end: '13:00', durationHours: 16.1, goalHours: 16, isGoalMet: true },
];
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-4 relative z-10">
                            <div className="p-4 bg-white/60 rounded-2xl backdrop-blur-sm border border-white/50">
                                ...
                                <div className="text-sm font-black text-slate-700">
                                    {status === 'fasting' ? startTime?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--'}
                                    <span className="text-slate-300 mx-1">→</span>
                                    {status === 'fasting' ? new Date(startTime!.getTime() + targetSeconds * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--'}
                                </div>
                            </div>
                            <div className="p-4 bg-white/60 rounded-2xl backdrop-blur-sm border border-white/50">
                                <div className="text-[9px] font-black text-slate-400 uppercase mb-1">Target Duration</div>
                                <div className="text-sm font-black text-emerald-600">{selectedPlan} Plan ({activePlan.hours}h)</div>
                            </div>
                        </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                        {PLANS.map(plan => (
                            ...
                            >
                                ...
                                <div className={`text-[9px] font-bold uppercase tracking-wider ${selectedPlan === plan.name ? 'text-emerald-50 opacity-80' : 'text-slate-300'}`}>
                                    {plan.desc}
                                </div>
                            </button>
                        ))}
                        ...
                        {sessions.slice(0, 3).map(s => (
                            <div key={s.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group active:scale-95 transition-all">
                                <div className="flex items-center gap-4">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${s.isGoalMet ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-200 text-slate-400'}`}>
                                        <History size={18} />
                                    </div>
                                    <div>
                                        <div className="text-sm font-black text-slate-800">{s.plan} Plan</div>
                                        <div className="text-[10px] font-bold text-slate-400 uppercase">{new Date(s.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-sm font-black text-slate-700">{s.durationHours.toFixed(1)}h</div>
                                    <div className={`text-[9px] font-black uppercase ${s.isGoalMet ? 'text-emerald-500' : 'text-slate-400'}`}>
                        ...
                        <Link href="/dashboard/learn?topic=fasting">
                            <Card className="p-5 flex gap-4 hover:border-emerald-200 transition-colors group">
                                ...
                                <div className="flex-1">
                                    <h4 className="font-black text-slate-800 mb-1 group-hover:text-emerald-500 transition-colors">Intermittent Fasting 101</h4>
                                    <p className="text-xs font-bold text-slate-400 line-clamp-2 leading-relaxed">Everything you need to know to start your journey today safely.</p>
                                </div>
                            </Card>
                        </Link>
                        <Link href="/dashboard/learn?topic=fasting">
                            <Card className="p-5 flex gap-4 hover:border-orange-200 transition-colors group">
                                ...
                                <div className="flex-1">
                                    <h4 className="font-black text-slate-800 mb-1 group-hover:text-orange-500 transition-colors">Fat Burning Secrets</h4>
                                    <p className="text-xs font-bold text-slate-400 line-clamp-2 leading-relaxed">How to optimize your metabolic window for maximum results.</p>
                                </div>
                            </Card>
                        </Link>
                    </div>
